...
class PongEnv:
    ...
    def reset(self):
        ...
        state = np.array([
            self.ball_x, self.ball_y, # aktualna pozycja pileczki 
            self.ball_dx, self.ball_dy, # o ile pilecczka sie przeimeszcza wzdluz x i y 
            self.r_paddle_y, self.l_paddle_y # aktualna pozycja paletki  
        ], dtype=np.float32)
        return state
    ...
    def step(self, action):
        ...
        #    self.l_paddle_y -= self.paddle_speed
        if np.random.rand() < 0.7:  # 70% szansy że w ogóle ruszy
        ...
        state = np.array([
            ...
            self.r_paddle_y, self.l_paddle_y
        ], dtype=np.float32)
        return state, reward, done

# --- Sieć Q-learning ---
def build_q_network(state_size=6, num_actions=3):
    model = Sequential([
        Input((state_size,)),
